...
import h5py
from apertools import constants, utils
from deformation import okada
import turbulence, stratified, deformation,decorrelation
import shutil
...
def generate_stacks(
    ...
    with ProcessPoolExecutor(max_workers=num_days) as executor:
        futures = [
            executor.submit(
                decorrelation.coherence2decorrelation_phase,
                coh,
                looks,
            )
...
def data_loader(
    ...
    #X = turb_stack/100
    X = noise*6 + defo_stack #/5
    # Make the DEM the first layer of the stack
    X = np.concatenate([dem[np.newaxis], X], axis=0)
    ...
    defo_stack = tf.reshape(defo_stack/3, (1, 96, 96, 16))
    defo_patches = tf.image.extract_patches(defo_stack,
                                             sizes=filter_dims,
                                             strides=filter_dims,
                                             rates=rates,
                                             padding='VALID')
    # reshape for needed sizes for SSIM
    defo_patches = np.array(defo_patches).reshape(16, 72, 128)
    defo_patches = defo_patches.reshape(-1, 144, 8, 8)

    noise_img = tf.reshape(noise, (1, 96, 96, 16))
    noise_patches = tf.image.extract_patches(noise_img,
                                             sizes=filter_dims,
                                             strides=filter_dims,
                                             rates=rates,
                                             padding='VALID')
    # reshape for needed sizes for SSIM
    noise_patches = np.array(noise_patches).reshape(16, 72, 128)
    noise_patches = noise_patches.reshape(-1, 144, 8, 8)

    # run SSIM for patches
    rms_defo = np.sqrt(np.mean(defo_patches ** 2))
    rms_noise = np.sqrt(np.mean(noise_patches ** 2))
    msnr = rms_defo ** 2 / rms_noise ** 2
...
def load_all_data(
    ...
    if sim_file_list is None:
        ...
        sim_file_list = sorted(sim_file_list, key=lambda i: int(re.split(r'_|\.', i)[-2]))
        dem_file_list = glob.glob(dem_file_glob)
        dem_file_list = sorted(dem_file_list, key=lambda i: int(re.split(r'_|\.', i)[-2]))
    X_all = None
    y_all = None
    snr_all = []
    from tqdm import tqdm
    for i in tqdm(range(len(sim_file_list))):
        Xs, ys, snrs = data_loader(
            sim_file_list[i],
            dem_file_list[i],
        ...
        if X_all is None:
            X_all = np.expand_dims(Xs, axis=0)
            y_all = np.expand_dims(ys, axis=0)
        else:
            X_all = np.concatenate((X_all, np.expand_dims(Xs, axis=0)))
            y_all = np.concatenate((y_all, np.expand_dims(ys, axis=0)))
        snr_all.append(snrs)
...
def save_predictions_as_tifs(
    arr, ds_file, idxs, outfile_template="{inp}_predicted_{d1}.tif"
):
    import xarray as xr
...
#        da = xr.DataArray(img, coords={"lat": lats, "lon": lons}, dims=["lat", "lon"])
       # da = xr.DataArray(img)
        ...
        outfile = outfile_template.format(inp=ds_file.replace(".nc", ""), d1=idx)
        print(outfile)
       # crs="EPSG:4326"
       # spatial_dims=("lon", "lat")
        
       # sario.save_xr_tif(outfile, da)
       # da.rio.write_crs(crs).rio.set_spatial_dims(*spatial_dims).rio.to_raster(outfile)
        im = Image.fromarray(img)
        im.save(outfile)


if __name__ == "__main__":
    dem_file = "./maduocut.tif"
    for idx in range (250000):
        generate_stacks(dem_file, outfile=f"./simulated_stack_{idx:02d}.h5")
        shutil.copy("./maduocut_{idx:02d}.tif")
    X, y = load_all_data(sim_file_glob="simulated_stack*.h5",dem_file=dem_file)
